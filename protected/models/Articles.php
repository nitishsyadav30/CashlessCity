...
class Articles extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id, article_name, article_content, author_name,article_active', 'required'),
			array('id, article_active', 'numerical', 'integerOnly'=>true),
			array('article_name, author_name', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('article_id, id, article_name, article_content, author_name, article_active', 'safe', 'on'=>'search'),
		);
	}
